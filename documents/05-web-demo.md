...
Webデモは`docfilly/styles.css`の公式オプトインCSSを読み込み、アプリ側ではツールバー高に合わせた`--docfilly-sticky-top`と`--docfilly-form-max-height`だけを上書きします。フォーム、入力欄、本文、Markdown、ライト／ダーク配色はCoreと同じスタイルを再利用するため、公開CSSとデモ表示が乖離しない構成です。
