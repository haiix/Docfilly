...
ツールバーの言語選択は現在のページセッションにだけ適用し、ブラウザーへ永続化しません。切り替えると`<html lang>`、Web UI、組み込みサンプルが更新され、Coreへもlocaleを明示的に渡すためdiagnosticの言語が切り替わります。利用者が開いた文書の本文、フォームラベル、選択肢、入力値は翻訳しません。
