...
ツールバーの「ヘルプ」は「Docfillyとは？」から始まり、Markdownやテキストへ入力項目と条件分岐を追加して利用者向けの本文を生成するDocfillyの役割を説明します。通常文書もフォームなしで表示できること、「文書を開く → フォームへ入力する → 自分向けの文書を読む」という基本の流れ、文書をブラウザー内で処理して外部サーバーへ送信しないことを、操作前に確認できます。

続いて、ファイル選択とドラッグ＆ドロップ、フォーム反映、2種類の保存、端末内保存とプライバシーを案内します。ヘルプには[ソースフォーマット仕様](./03-source-format.md)へのリンク、サンプル文書を開く操作、端末内の保存データを削除する操作もあります。PWA／オフライン起動は、現在利用できないことを明記しています。
