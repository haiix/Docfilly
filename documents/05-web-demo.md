...
ツールバーの「ヘルプ」から、ファイル選択とドラッグ＆ドロップ、フォーム反映、Docfillyと通常文書の違い、表示結果の書き出し、端末内保存とプライバシーを確認できます。ヘルプには[ソースフォーマット仕様](./03-source-format.md)へのリンク、サンプル文書を開く操作、端末内の保存データを削除する操作もあります。未提供のDocfilly形式保存とPWA／オフライン起動は、現在利用できないことを明記しています。
