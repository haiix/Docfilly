...
共有端末では、ヘルプの「この端末の保存データを削除」を利用してください。削除前には、復元用のファイル名、元ソース、形式、フォーム値が対象で、表示中の文書、元のローカルファイル、オフライン起動用のアプリデータは対象外であることを確認できます。削除しても現在の画面は閉じず、その文書を表示している間は再保存しません。別の文書またはサンプルを開くと自動保存を再開します。
...
ユーザーが開いた文書とフォーム値はService Workerへ渡さず、Cache Storageにも保存しません。前回文書の復元に使うIndexedDBとは保存先と用途を分離しています。アプリ内の「この端末の保存データを削除」はIndexedDBの復元データだけを削除するため、インストール状態とオフライン起動用キャッシュは維持されます。ブラウザー標準のサイトデータ削除では両方が削除されることがあります。
