...
Docfillyの設定を使う文書は、先頭の「識別子」、「設定項目エリア」、「本文エリア」で構成します。設定項目と本文は、半角ハイフン3文字の行で区切ります。
...
この行があるときだけ、Docfillyは区切り行より前を設定項目として解析します。通常は小文字で空白を付けず、上記のとおり記述してください。読み込み時は大文字・小文字を区別せず、前後の空白も許容します。UTF-8 BOMがあっても認識されます。
...
区切り行の前後には空白があっても構いません。分かりやすさのため、通常は`---`だけの行を推奨します。
...
現在のフォーマットにはエスケープ記法がないため、選択肢そのものにカンマを含めることはできません。空の選択肢が混ざっている場合は、その項目だけを読み飛ばして残りを使用します。すべて空の場合は、文書を止めずに通常のテキスト入力として表示します。
...
プレースホルダーには日本語を含む文字、数字、アンダースコアを使用できます。定義されていないプレースホルダーは削除されず、そのまま残ります。
...
Docfillyは、文書の一部に問題があっても描画処理を止めません。読み取れる設定項目と本文を使って表示を続け、`diagnostics`へ日本語の注意点を追加します。
