...
The app-owned interface and built-in tutorial support English (`en`) and Japanese (`ja`). With **Follow browser settings**, the viewer checks `navigator.languages` in order, then `navigator.language`, and falls back to English when none of those values is supported. Region variants such as `en-US` and `ja-JP` resolve to their base language.

Readers can switch languages in the toolbar's named **Settings** dialog. The versioned preference is stored in `localStorage`, separately from document recovery data, and persists in the current browser profile. Readers can return to **Follow browser settings** at any time. Changing the language updates `<html lang>`, all app-owned labels and messages, and the locale passed to Docfilly Core for diagnostics. An already opened document—including a built-in sample—keeps its source and current form values. Opening a sample afterward selects the sample for the current locale.

The same **Settings** dialog offers **Follow system settings**, **Light**, and **Dark** themes. The default follows `prefers-color-scheme` and reacts when the operating-system preference changes; an explicit choice is saved with the language preference. A small head script resolves the theme before the app renders, sets `color-scheme`, and updates the page's `theme-color` metadata to reduce reload flashing and keep browser controls consistent. The Web App Manifest keeps fixed light startup colors because installed-app manifest colors cannot follow a runtime preference; the live page metadata takes over after navigation.
