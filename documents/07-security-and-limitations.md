...
これにより、入力値へ`<script>`やイベントハンドラーを記述しても、実行可能なDOMとして挿入されないようにしています。元のMarkdown本文に含まれるHTMLも最終段階でサニタイズされます。
...
Docfillyは文書の構文上の問題で描画を停止しません。読み取れない設定行を飛ばす、最初の重複設定を使う、通常のテキスト入力へ置き換えるなど、安全な方向へ復旧します。

Markdown変換処理が予期せず失敗した場合も、元の出力ソースをプレーンテキストとして表示し、`markdown-render-fallback`の注意点を返します。

復旧内容は`diagnostics`へ格納されます。文書を作った人が修正できるよう、利用側アプリケーションではメッセージを表示してください。
